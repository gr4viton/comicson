...
#TODO make the effect change on touch and multitouch, text formula insertion
...
vec4 effect(vec4 color, sampler2D texture, vec2 tex_coords, vec2 coords)
{
    ...
    float dist_mag = (distance.y*distance.x + distance.x*distance.x*distance.y);
    vec3 multiplier = vec3(abs(cos(dist_mag + 10.0*time)));
